    ...
    link: function(scope, element, attrs) {
      ...
      if (angular.isDefined(scope.events) && scope.events !== null && angular.isArray(scope.events)) {
        var getEventHandler = function(eventName) {
          return function() {
            return scope.events[eventName].apply(scope, [map, eventName, arguments]);
          };
        };
...
.directive("marker", function($timeout, $log) {
  return {
    scope: {
      position: '=position',
      icon: '=icon',
    ...
    link: function(scope, element, attrs, mapCtrl) {
      ...
      scope.$watch(function() { return mapCtrl.getMap(); }, function(map) {
        marker.setMap(map);
        scope.map = map;
      });
      ...
      scope.$watch("icon", function() {
        marker.setImage(new daum.maps.MarkerImage(scope.icon.image, new daum.maps.Size(scope.icon.width, scope.icon.height)));
      });
...
.directive("markerLabel", function($timeout, $log) {
  return {
    restrict: 'E',
    require: '^marker',
    transclude: true,
    ...
    link: function(scope, element, attrs, markerCtrl, transcludeFn) {
      ...
      scope.$watch(function() { return markerCtrl.getMap(); }, function(map) {
        label.setMap(map);
      });
      scope.$on("$destroy", function() {
        label.setMap(null);
...
.directive("polyline", function($timeout, $log) {
  return {
    scope: {
      path: '=',
      strokeWeight: '=',
